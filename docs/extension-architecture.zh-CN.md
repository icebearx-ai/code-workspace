...
`seeded` 是 `file` 的一种特殊所有权：Host 只在目标文件缺失时写入扩展提供的默认内容（种子），之后把该文件视为用户所有。Host 负责：

- 首次安装且目标缺失时写入默认内容；
- 升级时永不覆盖已存在的用户内容，也不做漂移检查；
- 卸载时直接删除该文件，即使内容已被用户修改。

该能力用于「随扩展安装、之后归用户拥有」的本地配置文件（例如 monitor 的上报地址与开关）。
...
- 种子文件目标在首次安装后视为用户所有，升级不覆盖、卸载删除、不参与漂移检查。
