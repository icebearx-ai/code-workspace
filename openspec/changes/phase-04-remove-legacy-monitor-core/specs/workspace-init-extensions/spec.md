...
扩展 MAY 在 manifest 中声明抽象 `hooks`。Host SHALL continue to manage these Hooks through the extension package. Hook 必须使用 Host 支持的 provider-neutral
...
系统 SHALL 提供 `extension uninstall <name>` planned-write 命令。卸载 SHALL 只依据 installed 状态，由 Host 验证并移除独占文件、独占目录和共享 contribution，再删除该扩展状态；不得读取当前扩展包或执行扩展代码。完整后置条件验证前不得提交。Monitor 卸载不得依赖已删除的 legacy `src/monitor` 模块，也不得停止共享 Monitor service 或删除用户级运行数据。
