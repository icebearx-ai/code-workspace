...
`codew-add-projects` 和 `codew-resolve-branch` 目前由核心 managed files 直接维护，无法复用扩展的版本、事务和安装状态模型。将它们变为系统扩展，可以统一 Skill 资产的生命周期，同时保证它们随 Workspace 默认启用且不暴露为可选集成。

## What Changes

- **BREAKING** 将两个 Skill 及 `codew-add-projects` 的 Claude 命令从核心 managed files 迁移到 `extensions/.system` 下的版本化扩展包。
...
- 新增 `extensions/.system/codew-add-projects` 与 `extensions/.system/codew-resolve-branch` 包。
