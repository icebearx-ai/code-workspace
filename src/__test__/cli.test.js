...
test("init and update install native coordination Hook entries and preserve them", () => {
  ...
  assert.equal(yaml.load(fs.readFileSync(path.join(root, ".code-workspace", "config.yaml"), "utf8")).coordination.enabled, true);
  ...
  const disabled = run(root, ["update", "--no-coordination", "--json"]);
  assert.equal(disabled.status, 0, disabled.stderr);
  assert.equal(jsonData(disabled).coordination, false);
  assert.equal(yaml.load(fs.readFileSync(path.join(root, ".code-workspace", "config.yaml"), "utf8")).coordination?.enabled, false);
  assert.equal(commandFor(JSON.parse(fs.readFileSync(path.join(root, ".codex", "hooks.json"), "utf8")), "PreToolUse"), undefined);
});
